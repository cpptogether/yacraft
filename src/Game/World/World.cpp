...
void Game::World::Generate(int X, int Y) {
	...
	for (int X = 0; X < CHUNK_SEGMENT_SIZE; X++) {
		for (int Z = 0; Z < CHUNK_SEGMENT_SIZE; Z++) {
			int height = std::abs(Noise.GetCellular(worldXOffset + X, worldYOffset + Z) * 20);
...
void Game::World::BuildChunkModel(int chunkX, int chunkY) {
	...
	for (int segmentIndex = 0; segmentIndex < segments.size(); segmentIndex++) {
		const ChunkSegment& segment = segments.at(segmentIndex);
		for (int X = 0; X < CHUNK_SEGMENT_SIZE; X++) {
			for (int Y = 0; Y < CHUNK_SEGMENT_SIZE; Y++) {
				for (int Z = 0; Z < CHUNK_SEGMENT_SIZE; Z++) {
					const Block& block = segment.getBlock(X, Y, Z);

					if (block.getBlockTemplate() == Blocks::Air) {
						continue;
					}

					bool leftDrawable = true;
					bool rightDrawable = true;
					bool upDrawable = true;
					bool downDrawable = true;
					bool backDrawable = true;
					bool frontDrawable = true;

					int blockType = block.getBlockTemplate()->getType();

					if (X > 0) {
						if (segment.getBlock(X - 1, Y, Z).getBlockTemplate()->getType() == blockType) {
							leftDrawable = false;
						}
					}
					else {
					...
					if (Y > 0) {
						if (segment.getBlock(X, Y - 1, Z).getBlockTemplate()->getType() == blockType) {
							downDrawable = false;
						}
					}
					else {
						if (segmentIndex > 0) {
							if (segments[segmentIndex - 1].getBlock(X, 0, Z).getBlockTemplate()->getType() == blockType) {
								downDrawable = false;
							}
						}
					}

					if (Y < CHUNK_SEGMENT_SIZE - 1) {
						if (segment.getBlock(X, Y + 1, Z).getBlockTemplate()->getType() == blockType) {
							upDrawable = false;
						}
					}
					...
					if (Z > 0) {
						if (segment.getBlock(X, Y, Z - 1).getBlockTemplate()->getType() == blockType) {
							backDrawable = false;
						}
					}
					else {
					...
					float YOffset = (CHUNK_SEGMENT_SIZE * segmentIndex);

					if (upDrawable) {
						positions.emplace_back(inWorldX + (X + 0.5f), YOffset + (Y + 0.5f), inWorldZ + (Z + 0.5f));
						positions.emplace_back(inWorldX + (X - 0.5f), YOffset + (Y + 0.5f), inWorldZ + (Z + 0.5f));
						positions.emplace_back(inWorldX + (X - 0.5f), YOffset + (Y + 0.5f), inWorldZ + (Z - 0.5f));
						positions.emplace_back(inWorldX + (X + 0.5f), YOffset + (Y + 0.5f), inWorldZ + (Z + 0.5f));
						positions.emplace_back(inWorldX + (X - 0.5f), YOffset + (Y + 0.5f), inWorldZ + (Z - 0.5f));
...
void Game::World::Draw() {
	...
	WorldDrawShader.use();
	WorldDrawShader.setMat4(ProjectionSpaceInShader, Projection);
